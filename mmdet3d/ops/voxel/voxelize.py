...
def voxelization(points,
    ...
    if max_points == -1 or max_voxels == -1:
        ...
        voxels = torch.zeros(size=(max_voxels, max_points, points.shape[1]))
        coors = torch.zeros(size=(max_voxels, 3), dtype=torch.int)
        num_points_per_voxel = torch.zeros(size=(max_voxels, ), dtype=torch.int)
        voxel_num = torch.ops.voxelization.hard_voxelize(points, voxels, coors,
...
class Voxelization(nn.Module):
    ...
    def forward(self, points):
        ...
        return voxelization(points,self.voxel_size,self.point_cloud_range,self.max_num_points,max_voxels)
